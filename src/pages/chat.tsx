					...
					<div className="md:w-80 w-full h-screen bg-neutral text-base-content overflow-y-auto">
						<ul className="flex flex-row p-4 menu rounded-2xl">
							...
							{activeTab === 'channels' && channels.map((channel, index) => (
								<li key={index}
									className={selectedChannel === channel.id ? 'w-full cursor-pointer bg-primary text-white rounded-lg' : 'w-full cursor-pointer rounded-lg'}
										...
										<div className="dropdown dropdown-end">
											...
											<ul tabIndex={0}
												...
												<li>
													...
													<label
														onClick={() => handleDeleteChannel(channel.id)}>Delete</label>
												</li>
											</ul>
										</div>
									</div>
