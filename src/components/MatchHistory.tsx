...
export function matchInfo(user: string, matchHistory: any[]) {
	...
	for(let i = 0; i < total; i++) {
		if(user == matchHistory[i].player1.full_name) {
			if(matchHistory[i].score1 > matchHistory[i].score2)
				wins++;
			else if(matchHistory[i].score2 > matchHistory[i].score1)
				loses++;
		}
		else {
			if(matchHistory[i].score2 > matchHistory[i].score1)
				wins++;
			else if(matchHistory[i].score1 > matchHistory[i].score2)
				loses++;
		}
	}
...
export default function MatchHistory(props: any) : any {
	...
	useEffect(() => {
		axios.get(`/user/match-history/${props.id ?? user.user.id}/${props.limit ?? 5}`).then((res) => setMatchHistory(res.data));
	}, []);
