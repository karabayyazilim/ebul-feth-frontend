  ...
  return (
    <>
      <div className="flex md:flex-row flex-col gap-5 md:h-[calc(100vh-80px)] h-screen">
        <div className="flex flex-col md:w-3/12 md:h-full h-1/2 w-full items-center justify-center bg-neutral rounded-2xl border border-solid border-primary">
          <div className="flex w-full p-2 gap-x-2">
            <div className="w-full">
              <label
                htmlFor="openCreateChannel"
              ...
              {activeTab === "friends" &&
                friends?.map((friend, index) => (
                  <li
                    key={index}
                    className={
                      selectedFriend.id === friend.friend.id
                        ? "w-full h-auto cursor-pointer bg-primary text-white rounded-lg"
                        : "w-full h-auto cursor-pointer rounded-lg"
                    }
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-x-3">
                        ...
                        <div>
                          <span>{friend.friend.full_name}</span>
                        </div>
                      </div>
                  ...
                  <li
                    ...
                    className={
                      selectedChannel === channel.id
                        ? "w-full cursor-pointer bg-primary text-white rounded-lg"
          ...
          <div className="card w-full bg-neutral shadow-xl h-[400px] md:h-full">
            <div ref={chatRef} className="card-body overflow-y-auto">
              ...
              {Object.keys(selectedFriend).length ? (
                <div>
                  {directMessages.map(
                    (direct: IMessage, index: number): React.ReactElement => (
                      <div
                        className={
                          direct.recieverId !== user!.id
                            ? "chat chat-end"
                            : "chat chat-start"
                        }
                        key={index}
                      >
                        ...
                        <div className="chat-header">{direct.name}</div>
                  ...
                  {channelsMessages.map(
                    (
                      ...
                      >
                        ...
                        <div className="chat-header">{channelMessage.name}</div>
                        <div className="bg-green-950 chat-bubble">
      ...
      {selectedChannel && (
        <ChannelSettingModal channelId={selectedChannel} friends={friends} />
      )}
      <CreateChannelModal />
      <AddFriendModal />
    </>
